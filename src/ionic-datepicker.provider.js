        ...
        if ($scope.mainObj.lanq) {
          ...
          for (var i = $scope.tempFirst; i <= $scope.tempLast; i += 86400000) {
            ...
            if(tempDate.toString().match(/([A-Z]+[\+-][0-9]+.*)/)[1] == 'GMT+0430 (Iran Daylight Time)')
             ...
             else if(tempDate.toString().match(/([A-Z]+[\+-][0-9]+.*)/)[1] == 'GMT+0330 (Iran Standard Time)'  && new Date(tempDate.getTime()-86400000).toString().match(/([A-Z]+[\+-][0-9]+.*)/)[1] == 'GMT+0430 (Iran Daylight Time)'){
               tempDate = new Date(tempDate.getTime()+3600000);
             }
            $scope.dayList.push({
